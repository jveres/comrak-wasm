...
pub fn parse_document<'a>(arena: &'a Arena<'a>, md: &str, options: &Options) -> Node<'a> {
    parse_document_impl(arena, md, options, None)
}

/// A lexical leaf captured before text coalescing. Columns are UTF-8 bytes.
#[derive(Clone, Debug)]
pub struct SourceLeaf {
    /// Parsed value, including decoded entities and smart punctuation.
    pub value: NodeValue,
    /// Original source range before adjacent text nodes are joined.
    pub sourcepos: Sourcepos,
    /// Source columns of literal lines, including the opening fence's info line.
    pub line_offsets: Vec<usize>,
    /// Source line corresponding to the first entry in line_offsets.
    pub line_start: usize,
}

/// Parse once while retaining lexical source spans before text coalescing.
pub fn parse_document_with_source_map<'a>(
    arena: &'a Arena<'a>,
    md: &str,
    options: &Options,
) -> (Node<'a>, Vec<SourceLeaf>) {
    let mut leaves = Vec::new();
    let root = parse_document_impl(arena, md, options, Some(&mut leaves));
    (root, leaves)
}

fn parse_document_impl<'a>(
    arena: &'a Arena<'a>,
    md: &str,
    options: &Options,
    leaves: Option<&mut Vec<SourceLeaf>>,
) -> Node<'a> {
    ...
    let document = Parser::new(arena, root, options).parse(md, leaves);
    ...
    'c: 'o,
{
    ...
    fn parse(mut self, mut s: &str, leaves: Option<&mut Vec<SourceLeaf>>) -> Node<'a> {
        ...
        if let Some(leaves) = leaves {
            for node in self.root.descendants() {
                let ast = node.data();
                let mapped_leaf = matches!(
                    ast.value,
                    NodeValue::Text(_) | NodeValue::Code(_) | NodeValue::CodeBlock(_)
                );
                #[cfg(feature = "shortcodes")]
                let mapped_leaf = mapped_leaf || matches!(ast.value, NodeValue::ShortCode(_));
                if mapped_leaf {
                    let parent = node
                        .ancestors()
                        .skip(1)
                        .find(|n| !n.data().line_offsets.is_empty());
                    let (line_offsets, line_start) = if matches!(ast.value, NodeValue::Code(_)) {
                        parent
                            .map(|n| {
                                let data = n.data();
                                (data.line_offsets.clone(), data.sourcepos.start.line)
                            })
                            .unwrap_or_default()
                    } else {
                        (ast.line_offsets.clone(), ast.sourcepos.start.line)
                    };
